...
import::here(read_species_list, read_grid, build_pa_dataset,
             load_or_make_folds, .from = "R/01_helpers_data.R")
...
list(
  ...
  tar_target(grid_df,     read_grid(here("data", "tas_sdm_map.csv")), format = "file"),

  # ── build PA data ────────────────────────────────────────────────────────
  tar_target(pa_df,       build_pa_dataset(species_tbl, grid_df, min_species = 3)),

  # ── cross-validation folds ───────────────────────────────────────────────
  tar_target(folds, load_or_make_folds(pa_df,
               here("data", "spatial_folds.rds")), format = "file"),

  # ── modelling (simple split: fold == "train") ────────────────────────────
  tar_target(train_df, pa_df |> filter(fold != 1, pa %in% c(0, 1))),
  tar_target(test_df , pa_df |> filter(fold == 1)),
